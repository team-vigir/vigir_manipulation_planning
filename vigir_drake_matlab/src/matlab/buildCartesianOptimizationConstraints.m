function activeConstraints = buildCartesianOptimizationConstraints(robot_model, request, waypoints, q0)
    ORIENTATION_FULL = 0;
    ...
    torso_upright_constr = WorldGazeDirConstraint(robot_model, torso_body_idx, [0; 0; 1], [0;0;1], pi/4, [waypoints(1).waypoint_time, waypoints(end).waypoint_time]);
    ...
    if ( strcmp(waypoints(1).target_link_names, 'l_foot' ) == 0 )
    ...
    if ( strcmp(waypoints(1).target_link_names, 'r_foot' ) == 0 )
    ...
    for waypoint_idx = 2:length(waypoints)
        start_waypoint = waypoints(waypoint_idx-1);
